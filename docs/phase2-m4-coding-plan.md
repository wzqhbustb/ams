...
| **A1 回收页撕页修复（pg-storage,2026-08-31 用户决策入 M4 scope,ROADMAP 附录 A1)** | freelist 复用页不再享受"新页无旧镜像"假设（`new_page` 对复用页仍设 `needs_fpi=false`，其前任镜像在盘上，纯断电模型才暴露）；修复按 stage_spec:879 候选（split_prepare 对复用右页补 `log_page_init`；实现期审计后可在页初始化路径统一补，二选一）+ 崩溃注入回归（复用页 + kill -9 + 恢复一致性）。**动既有 crate 的理由（工程规则要求）**:Phase 2 新 AM 上线后页回收更频繁、暴露概率上升；修复面小且与 pg-am-hnsw 零耦合，A1 修完后 Phase 7a 加固专项该项划销。回归 = `cargo test --workspace` 全量 + `crash_recovery` 系列 |
