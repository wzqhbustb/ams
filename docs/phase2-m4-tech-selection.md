...
| v1.19 | 2026-09-03 | Stage D 开工 + 第二轮外审回流（以代码为准）:dataset 模块加固——伪造 dim 预算闸门（> u16::MAX 在分配前响亮拒）/ metadata 后整记录截断检测（记录数精确比对）/ `recall_at_k` 完整性校验（base_count 参数，gt 与 retrieved 的越界、重复、短行全 fail loud;u32 对比较，根除 `as i32` 回绕）;probe 对齐 §12 冻结口径（预热 1 轮 + 计时 3 轮取中位；percentile 最近秩修正，原 P99 实为 max）+ 参数严格解析 + 全参数输出（A/B 单变量可证）+ `drop(base)` 削 gist 内存峰值（16GB runner 判断随之修正：快照段峰值 ≈ 12–13GB，紧张但可能可行，OOM 则 gist 拆档）;数据管道——fetch 脚本套娃修复 + SHA-256 钉死 + 原子发布 + 完成标记，recall-gate job 注册进 ci.yml（五件清单），ftp 探针 verdict 修无效结论漏洞 |
| v1.20 | 2026-09-07 | Stage D 第三轮外审回流（以代码为准）:fetch 脚本 `.done` skip 不再架空摘要——skip 前与当前期望 SHA-256 比对，STALE 即清理重走（下载后校验 fail loud）;probe `M4_SNAPSHOT` 显式 0|1（原 `==1` 静默吞掉非法值）;dataset 双轨预算 API（`read_*_with_budget`，读前字节闸门，unlimited 封装仅限可信本地——与 §7 snapshot 预算同一威胁模型） |
| v1.21 | 2026-09-07 | Stage D 第四轮外审回流（以代码为准）:dataset 读取 TOCTOU 封顶（`take(min(budget, file_len)+1)` + EOF 后增长探针，RSS 物理封顶；无限流用例钉死）+ 非普通文件闸门（/dev/zero/FIFO/目录响亮 InvalidArgument，对齐 §7 snapshot 闸门语义）;fetch skip 核验 payload 完整性（三文件存在非空才许跳过，缺失自愈重解压） |
| v1.22 | 2026-09-07 | Stage D 第五轮外审回流（以代码为准）:fetch `.done` v2 清单（逐文件 SHA-256+尺寸）——skip 核验升级为逐文件哈希比对（截断/同长度篡改均可检出自愈）;dataset 字节预算 ≠ 内存预算写入 rustdoc（Vec-of-Vec 放大上界公式 + 64MB dim=1 实测 ~5×;dim ≥ 128 ~1.01×)，专用内存预算登记残留——与 §7 snapshot 双预算同教训 |
| v1.23 | 2026-09-08 | Stage E 对抗审查两轮回流（以代码为准）:**§7 内存闸门修正**——`max_memory_estimate` 口径明确为"堆物化上界"，闸门另加文件映像项（`read_cap` 计费）;body 读法改 `Vec::with_capacity(file_len+1)` 精确预留（不修 `read_cap_total`——伪造头可将其吹到 ~1e17 致瞬时分配 abort，论证入注释）；外审场景（dim=8192/m=2/N=4000）实测峰值 460→265MB、闸门 412.7MB 上界重新成立，第二轮复核边界（恰低拒/恰等放行）实测通过。criterion bench 进 dev-deps(criterion 0.5 复用 lockfile 钉版，运行时依赖面不变：thiserror + crc32fast) |
