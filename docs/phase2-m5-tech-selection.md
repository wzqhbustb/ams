...
- redo handler 接线点对齐 btree 先例：pg-am-hnsw 导出
  ...
  `Engine::open` 注册（pg-engine/src/engine.rs:692-693 现有
...
(a) **生理记录 on 节点页**——与全部现存 AM 同构：页+槽位寻址、pd_lsn
    权威幂等（pg-storage/src/wal/record.rs:238 的 heap 先例）、FPI/checkpoint/
    崩溃测试基建全部白拿；
...
| 124 | HnswNodeTombstone | (page, slot, node_id, dim, meta_page_id) —— 格式交付，语义 M6 生效（§1);**dim 定位状态字节**（偏移 4·dim,v1.17——redo handler 无状态，payload 不含 dim 则位不可寻）;**meta_page_id 供 redo handler 自定位 meta，应用前核对 dim == meta.dim**(v1.18 入 payload;v1.19 起一致性核对从 §11.3 审计上移为 redo 前置门——审计拿不到历史 payload，错误 dim 留到审计才核对则 redo 早已改坏条目） |
| 125 | HnswDirAppend | (目录尾页， node_id → (page u64, slot u16) 条目后像，条目宽 10B)——**真单页记录**(v1.2) |
| 126 | HnswDirLink | (旧尾页， next_page_id 链指针后像）——目录扩容专用，单页 |
| 127 | HnswPublishLive | (page, slot, node_id, dim, meta_page_id)——state 后像翻 LIVE(v1.7，见下）;dim 的定位作用同 124(v1.17),meta_page_id 的自定位作用与 redo 前置门同 124(v1.18/v1.19) |
...
(record.rs:1627-1636),RedoContext 无索引定位能力（recovery.rs:210-229
...
Tombstone 与 PublishLive 另携带 **dim**(v1.17——状态字节偏移 4·dim
的唯一无状态来源，缺它则 redo 对这两条记录物理不可施行）与
**meta_page_id**(v1.18——redo handler 从 payload 自定位 meta 页；
v1.19 起 dim == meta.dim 的核对随之上移为 **redo 前置门**：handler
读 meta 核对后才应用，不再降级 §11.3——审计拿不到历史 payload，留
到审计才核对的错误 dim 早已在 redo 期改坏条目；redo 可求值的前提 =
meta 初始化记录在 LSN 序上先于一切节点记录且 dim 不可变）;
见 §10.1 纵深校验；对齐 btree
记录 payload 带 page_id 的先例
record.rs:1223)。redo 时 meta page 经 ctx.buffer_pool 可读（Stage I 起
...
- **幂等锚 = pd_lsn**(`page.pd_lsn >= record.lsn` 跳过，heap 先例
  record.rs:238)；页内内容 authoritative，无读时依赖。
- **payload 版本**:payload 版本 nibble 在 **u8** flags 的高 4 位
  (`version = flags >> 4`,record.rs:941-946——注意 M1 冻结的 32B 记录头里
  flags 是 u8，不是 u16)。HNSW 记录从 v0 起步；**124/127 自 v1.18 起
  版本化为 v1**(`HNSW_STATE_VERSION_V1`/`HNSW_STATE_V1_FLAGS`——
  payload 两次格式修订（+dim、+meta_page_id）后补打的版本通道；
  flags=0 的预版本化开发格式与未知版本均响亮拒绝，不做兼容解码，
  其余五类型格式未变、flags=0 即隐式原始版）。
...
最大层是单值字段，append 式历史对它们无意义）。**meta page 物理形态 =
32B PageHeader + raw 字段区**(2026-09-14 Stage A 审查 P3-2 收窄：上文
"slotted page"仅指共享页头——**line-pointer 数组在 meta page 上永不
使用**，否则 LP[0] 与字段区首字段（entry_point@32）直接混叠；
entry_point@32 / max_level@36 为格式常量，Stage A 已冻结）,redo handler
不复用 btree 的。
...
- **容量不变量**(v1.1 审查 P1-1 重写；v1.5 P1-2 后公式由"最坏情况"变为
  **精确值**——每个条目都按满载预留）。页可用面 = PAGE_SIZE − 32
  (PageHeader)− 4(LinePointer——**LP 布局归属**(2026-09-15 Stage B slice 1
  裁断落地）:LP 布局（off:15|flags:2|len:15,LP_NORMAL=1）**已上移
  pg-storage::page 为新消费方单一真源**(`LINE_POINTER_SIZE`/
  `LP_NORMAL`/`encode_line_pointer`/`decode_line_pointer`,golden pin
  0x00C0_9F40 双侧钉）；历史属主 pg-am-heap/src/line_pointer.rs:16 的
  统一迁移登记为未来重构（本阶段不动 heap——最小变更）;
...
- **redo 侧纵深校验——冻结清单**(v1.1 审查第二轮 P3-3③ 立、v1.6 P3-3
  ...
  - **HnswPublishLive**：目标条目存在，state ∈ {INITIALIZING, LIVE}
    ...
    通过后才应用；meta 初始化记录在 LSN 序上先于一切节点记录且
    dim 不可变，故 redo 可求值；open 审计拿不到历史 payload,
    v1.17/v1.18 的"同型降级审计"口径作废——留到审计才核对的错误
    dim 早已在 redo 期改写错误字节）;("node_id 与目录映射一致"
    降级到 §11.3 审计，同上）;
  - **HnswNodeTombstone**：目标条目存在且 LIVE(M5 只重放不生效语义，
    §1);M6 生效时再按那时口径收紧。dim == meta.dim 同为 redo 前置门
    (v1.19，可求值前提与作废口径同 PublishLive)。
  此清单为 M5 redo 校验的**冻结边界**：新增检查项 = 协议修订记录；更深
  的图级校验（连通性、目录一致性、HWM 导出核对——含上述四项降级项：
  被引 id < HWM、entry_point < HWM、PublishLive/owner 两条目录一致性）
...
- `buffer_pool: Option` 的处置（v1.1 审查 P2-4 改写——v1.0 引用了"M1
  ...
  recovery 恒在 buffer pool 打开后进行**（pg-storage/src/engine.rs:237-245
...
1. **from_u8 判别值注册**:record.rs:130 的 `from_u8` 加 121–127 分支
   （未知判别值现有行为 = WalReadFailed 硬失败，保持）;
   tests/wal_record_type_discriminant.rs 钉表同步新增；
2. **DPT touched-page 分类**:analysis.rs:269 的 `for_each_touched_page`
   ...
   未注册时该测试变红，无需另写护栏；
3. **pg-waldump 解码**:pg-waldump.rs:365 现状把 LogicalHnsw 列在
...
2. **物理应用原语**(pub(crate) 起步，v1.7 形态——v1.7 审查 P2-1：签名
   ...
   **签名口径**(2026-09-14 Stage A 落地裁断①):v1.7 的"物理页参数"落地
   为**页 buffer**——纯应用原语不做 I/O,payload 携带的 PageId 由调用方
   （redo handler pin / 正常路径）解析为 buffer;append_node/apply_node_at
   因定长分档另带几何参数（dim/m/m_max0，落地为 `NodeGeometry` 值对象）;
   redo 形态分化为 `append_node`（正常路径追加分配）与
   `apply_node_at(slot, …)`(redo：空闲创建 / INITIALIZING 覆写幂等，即
   §10.1 "目标 slot 为空闲或 INITIALIZING"的实现载体，Stage A 审查 P3-1)。
   **落地签名**(2026-09-14 Stage A 二轮复核 P3-1 按代码重写，取代 v1.7
   bullet——参数语义：页 = 页 buffer;`geo` = NodeGeometry(dim/m/m_max0);
   owner/count/无自环在 funnel 不在原语）:
   - `select_slot(node_page, len) -> slot` — **非修改式选槽**(v1.17,
     Stage A 三轮 P1-1——WAL-first 次序的载体：正常路径先选槽、把 slot
     写进 WAL 记录、append+flush，然后才 `apply_node_at`;append_node
     改为此两者的组合，单一实现）;
   - `append_node(node_page, node_id, top_level, geo, vector) -> slot` —
   ...
   - `publish_live(node_page, slot, dim)` — state 后像翻 LIVE（步骤 8);
   - `dir_append(dir_tail_page, node_id, target_page, target_slot)` — 尾页
     单页条目写（步骤 4);**node_id 键控幂等**(v1.17, Stage A 三轮
     P3——node_id == 链 HWM:`hwm == id` 追加、`hwm > id` 幂等跳过、
     `hwm < id` 间隙响亮，原语自身即 N=3 字节全等，不只靠 handler 的
     pd_lsn 守卫）;
   - `dir_link(old_tail_page, new_dir_page)` — 目录扩容（步骤 2);
   - `apply_meta(meta_page, entry_point, max_level)` — 步骤 7 与 §10.3
     的创建/修复共用；
   - `apply_tombstone(node_page, slot, dim)` — HnswNodeTombstone(124)的
     redo 承载（v1.9 审查 P2-1 补登）;tombstone 语义 M6 才生效，但 §1
     划的 M5 范围是"能写、能重放、能校验"——重放承载的原语必须在
     清单内;
   **读侧热路径**(v1.17, Stage A 三轮 P3):`neighbor_iter`/`vector_iter`
   零分配借用迭代器为 Stage C 搜索形态，`entry_neighbors`/`entry_vector`
   是其 `.collect()` 便利封装（每次调用分配 Vec，不进热路径）;
...
**写序**(v1.31，用户终审六轮 P1):pin_mut（触发到期的 pre-image FPI)
→ append MetaUpdate → apply → stamp pd_lsn,btree write_meta_record
同款；反序会使 FPI 的 LSN 高于 MetaUpdate,redo 先放 MetaUpdate 再放
修复前映像，修复被冲掉且 pd_lsn 滞后于 FPI LSN(pd_lsn 权威违约）。
**发布前检查**(v1.31，六轮 P3):节点条目读出的 top_level 是 6-bit
掩码（≤63 仅格式上界），必须 ≤ l_max(m)(next_level 抽取的语义上界，
任何抽取都不可达之——(l_max(m), 63] 即定义性腐坏）;read_meta 对
meta 页存储的 max_level 镜像同检查（HnswMetaUpdate 构造器拿不到 m,
只守 6-bit 格式上界）。
...
pd_lsn 守卫（record.rs:238 模式）+ 后像覆写使这天然成立，测试是钉不是证。
...
| v1.14 | 2026-09-14 | M5 Stage A 落地回流 + 审查裁断回写（agent-23 verdict PASS 附条件，条件项已闭合，详见 coding-plan v1.13 与 stage_spec Stage A 回流段）。**裁断①（原语签名口径）**:§10.2 补——v1.7"物理页参数"落地为页 buffer（纯应用原语不做 I/O,PageId 由调用方 pin 解析）;append_node/apply_node_at 带几何参数（落地为 `NodeGeometry` 值对象）;redo 形态分化为 append_node（正常路径）与 apply_node_at(slot,…)（空闲创建/INITIALIZING 覆写幂等，§10.1 "目标 slot 空闲或 INITIALIZING"的实现载体）。**裁断②(LP 布局归属）**:§7.2 容量不变量补注——LP 布局由 pg-am-hnsw apply.rs 再导出（§2 禁依赖 pg-am-heap 的推论）,golden pin 测试冻结一致性，上移 pg-storage::page 登记为 Stage B 评估候选。**P3-2(meta page 形态收窄）**:§6 "slotted page"收窄为"32B PageHeader + raw 字段区，LP 永不使用";entry_point@32/max_level@36 为格式常量（Stage A 已冻结）。**P3-3(slot 态校验归属）**:funnel 的 slot 态 ∈ {INITIALIZING, LIVE} 项需页访问，归 Stage C handler 侧（validate.rs 注释立文） |
| v1.15 | 2026-09-14 | M5 Stage A 二轮复核回流（agent-23 verdict PASS 附条件 → 条件项已闭合，详见 coding-plan v1.14)。**P2-1（页内容边界）**:pd_lower/pd_upper/LP off+len 无 PAGE_SIZE 钳制（页无 checksum,bit-rot 可致 redo 路径 slice panic，违反 no-panic 纪律）——read_lp 入口钳制（pd_lower ∈ [32, PAGE_SIZE] 且 4 对齐）+ 返回前 off/len 钳制，append_node/apply_node_at 增 pd_upper ≤ PAGE_SIZE 守卫。**P3-1**:§10.2 原语 bullet 列表按落地签名重写（v1.7 遗物与 2026-09-14 口径段双重矛盾：append_node 无 meta_page_id/slot、set_neighbors 无 owner/count、publish_live/apply_tombstone 以 dim 为参、apply_meta 以页 buffer 为参）。**P3-2(裁断）**:set_neighbors/entry_neighbors 统一为 `NodeGeometry` 签名（与 append_node/apply_node_at 一致，API 单约定）。**P3-3**:LP golden pin 的 encode 断言改字面量 0x00C0_9F40（公式化期望与实现同式自指，守卫失效）。**nano**:l_max 补 m=2 → 53 边界钉（rng.rs:93-96 上界语义） |
| v1.16 | 2026-09-14 | M5 Stage A 三轮复核回流（用户终审 P3：源码行号漂移，逐项核实修复）。正文锚点更新：from_u8 record.rs:107-138→:130、btree_insert 构造器先例 :891-904→:1223、for_each_touched_page analysis.rs:267→:269、pg-waldump HNSW 解码臂 :336→:365、Engine::open extend 链 engine.rs:687-689→:692-693（两处）、pg-storage recovery 阶段序 engine.rs:236-243→:237-245、heap pd_lsn 幂等先例 record.rs:209→:238（三处）、WalRecord 头布局 :705-718→:1627-1636、version nibble :638-643→:941-946；pg-am-hnsw 两处源码注释同步（redo.rs:4、page.rs:105 的 PageHeader::write_to :118-128→:142-153)。核实不改：analysis.rs:854 穷举测试、engine.rs:646 的 RedoContext 构造、recovery.rs:210-229、record.rs:74/:81-88/:189 仍准确；修订记录表历史行号按 v1.12 起约定保留当轮值。coding-plan v1.15、stage_spec :1171 同步 |
...
| v1.21 | 2026-09-15 | M5 Stage B slice 1 落码回流。**LP 布局上移裁断落地**(Stage A 裁断②的 Stage B 评估结论：做）:pg-storage/src/page.rs 新增 `LINE_POINTER_SIZE`/`LP_NORMAL`/`encode_line_pointer`/`decode_line_pointer`(pub，布局单一真源；历史属主 pg-am-heap 的迁移登记为未来重构，本 stage 不动）,golden pin 0x00C0_9F40 双侧钉；apply.rs 删本地 LP 常数改消费，read_lp/write_lp 换 helper。**node.rs 新建**(pub(crate),§7.2 节点条目格式属主）:STATE_LIVE/TOMBSTONE_BIT、NodeGeometry、entry_size/level_region_size/level_offset/level_base_offset/level_region_pos/state_offset 自 apply.rs 迁入；新增 `NODE_PAGE_USABLE`(PAGE_SIZE 参数化，8KB 下 8156)与 `check_creation_geometry`(dim/m/m_max0 乘积联动创建时硬校验，最坏 L_max=⌊53·ln2/ln m⌋ 满载反解；默认参数 dim ≤ 1791、1792 拒、dim=0 拒、m=2 的 L_max=53 边界联动——m_max0 写死值不存在，公式即不变量）。**dir.rs 新建**(pub(crate),§7.1 目录链属主）:`DIR_ENTRIES_PER_PAGE` 公式化迁入（=813) + `DirChainInfo{page_count,hwm}` + `check_dir_chain`(fetch 注入纯函数——链结构四断言：ordinal 连续自 0/中间页恰满/next 唯一无环（Vec 查环，禁 HashMap)/count ∈ [0,813] + 页类型与 sanity 上限护栏;hwm = tail.ordinal × 813 + tail.count,checked 算术；§11.3 审计与 open 时 HWM 导出同函数单实现）。新增测试：pg-storage page +1(LP golden+往返）、node.rs +3（位常量/分档/创建几何）、dir.rs +2（合法链推导/七类畸形响亮）。验证：pg-am-hnsw lib 123 绿（117+6)、pg-storage lib 208 绿（207+1)、clippy -D warnings/fmt/doc 两 crate 绿；coding-plan v1.20、stage_spec Stage B 节同步 |
| v1.22 | 2026-09-15 | M5 Stage B slice 2 落码回流:**§6 字段表落码**——meta.rs 新建为 meta page 布局属主，冻结偏移表(v1 格式：entry_point@32/max_level@36/meta_format_version/metric/selection/dim/m/m_max0/ef_construction/ef_search_default/reserved/rng_seed/dir_head/snapshot_format_version;前两位 Stage A 冻结未动）;**metric/NeighborSelection 首次持久化**(graph.rs 判别值 0=L2/1=Cosine/2=IP、0=Heuristic/1=Simple，未知响亮 Corrupted——§6 残留的"快照不带则页驻必须自描述"兑现）;**ef_search_default 的 WARN 口径落地**:check_expected 的六硬错集(dim/m/m_max0/efC/metric/selection)InvalidArgument 带字段名两侧值,ef_search_default 错配 → `Vec<String>` warnings 返回不失败(v1.4 nano 落地,**无 tracing 依赖**——crate 依赖冻结,warnings 以字符串由调用方处置）;read_meta 11 项结构校验（创建几何单一规则调 node::check_creation_geometry，不重写）。验证:pg-am-hnsw lib 126 绿（123+3)、clippy/fmt/doc 绿；coding-plan v1.21、stage_spec Stage B 节同步 |
| v1.23 | 2026-09-16 | M5 Stage B slice 3a 落码回流:§10.3 创建/open 协议按本文落码——创建序(dir 首页 → meta 页,各 init + log_page_init)与崩溃窗口记账一致;open 修复三性(幂等/确定性/正常 WAL 记录)与"备选及不选"论证原样落成 rustdoc;skip-ahead 按 HWM 精确重播(§5 (c) 案)。落码事实补记:残态测试的节点/目录内容以 FPI 镜像形式落成(121–127 逻辑记录重放依赖 Stage C handler,该验证登记为 Stage C 测试依赖);`dir_entry` 读取助手随 open 修复落地。coding-plan v1.22、stage_spec slice 3a 段同步 |
| v1.24 | 2026-09-16 | M5 Stage B slice 3b 落码回流：**§10.3 步骤 3（引擎侧 first_page 登记）的最小登记口径实录**——create_hnsw_index **只写 `pg_rust_relpages` 一行** `(oid, meta_page, meta_page, 1)`（btree 同款 heap AM/auto_commit 路径；**无** pg_class/pg_attribute/pg_index:M5 无 SQL 用户面，孤行被 registry 重建跳过策略兼容（§10.3 范围调和兑现，SQL/DDL 归 Phase 4);hnsw_index_first_page 经 **heap AM 扫描** 读回（Catalog 内存缓存只在 engine open 加载一次，create 后立查为 None——读路径教训实录）;open_hnsw_index 薄封装，ef_search_default 的 WARN 口径经 OpenOutcome.warnings 透出；EngineError::Hnsw(#[from] pg_am_hnsw::HnswError) 错误通道变体（与 BTree 变体同风格）。验证：pg-engine tests/m5_hnsw_create_open 2/2 绿（含引擎重开耐久）、pg-am-hnsw lib 127 绿无回归、clippy/fmt/doc 绿;coding-plan v1.23、stage_spec slice 3b 段同步。Stage B(页布局 + 目录链 + meta page + 创建协议）落码面齐，待对抗审查 |
| v1.25 | 2026-09-16 | M5 Stage B 对抗审查回流(agent-23 verdict PASS 附条件 → 闭合):P3-2 `dir_entry` count 钳制与 Stage A round-2 页内容钳制纪律同族(§7.1 读取侧全部入口自此带页内容边界);P3-1 页化 meta 进 funnel 的 From 桥落实(§10.1 的 Stage C 消费点成形);nano:create 耐久边界(§8.1 成功边界① 对创建协议同样成立——create 返回 ≠ 耐久,flush_to/commit 是边界)。无格式/协议变更;coding-plan v1.24、stage_spec Stage B 三小节同步 |
| v1.26 | 2026-09-16 | M5 Stage B 主线独立复核回流:**P3-1** §6 meta page 结构校验补三条同班项——max_level 域(≤63,与 §7.2 的 6-bit top_level 同一口径)、entry_point=INVALID ⇒ max_level=0(§4.2 MetaUpdate 构造规则的读侧镜像)、ef_search_default ≥ m(构造域规则;v1.4 nano 的 WARN 口径只管调用方错配,不管域外值)——meta page 校验自此与写侧规则完备互镜;coding-plan v1.25、stage_spec 主线复核回流段同步。无格式/协议变更 |
| v1.27 | 2026-09-16 | M5 Stage B 主线复核二轮回流:**P2-1** §10.3 open 修复与 Stage 0 空骨架的交互坐实——修复写 123 后无 checkpoint 的重开硬失败(fail-loud;§10.1 handler 落地即消,登记为 Stage B 已知残留"重开限制",Stage C 第一个任务闭合);**P3-1** §7.1 链导出 HWM 补 u32 NodeId 空间护栏(§3 契约的边界对齐,hwm > u32::MAX 响亮);coding-plan v1.26、stage_spec 二轮回流段同步。无格式/协议变更 |
...
| v1.40 | 2026-09-21 | M5 Stage C slice 4(页驻查询)落码回流:① **§10.2 任务 3 消费面闭合**——页驻 search 落成(HnswIndex::search),两形态共享泛型核的既定重构自此两端俱在;错误优先级逐条镜像内存孪生(dim → §5 → ef ≥ k → 空图 → k=0),Outcome 级对拍钉死(结果逐位等或错误变体同)。② **P3-B 闭合**(slice 3 终审锐化的评估点,结论 = 做):distance.rs 迭代器距离变体(位级一致 = 同分量序/同提升/同累加器序列,to_bits 钉)+ Metric::distance_iter 单点分发;dist_to_query 零分配、dist_between 一分配(死锁纪律禁双 pin);受控切片重解释(unsafe)方案否决——安全 Rust 纪律优先,Stage E benchmark 若证分配为瓶颈再议。③ §8.1③ INITIALIZING 搜索语义落成并经残态召回测试钉死(state 位 = 恢复进度标记,非可见性过滤)。无格式/协议变更。coding-plan v1.42、stage_spec slice 4 段同步 |
