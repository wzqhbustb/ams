...
| F7 排期登记（Stage F 终审建议） | "commit() 失败路径泄漏 XID"（engine.rs:528-542：commit 失败时 handle 已消耗、XID 永留 active set 钉住 horizon）是 engine 侧真问题——本 stage 收口时显式排期处理（修复点：commit 失败时 best-effort abort 兜底，对齐 Stage O 的 commit 失败语义），不随 M3 关闭沉没 |
