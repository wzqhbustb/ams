...
| 任务 | 交付物 |
|------|--------|
| SegmentedStorage 预留 | `pg-storage` 新增 `SegmentedStorage` trait（`create_segment/freeze/seal/merge`）+ `SegmentState { Active, Frozen, Sealed, Merging, Retired }` 单向状态机（§8）；`SegmentSeal=110`/`SegmentMerge=111` 判别式 **Stage 0（M1+M2 基线）已预留**（record.rs:80-83），本 stage 只写 payload doc 契约（segment id 列表、目标 id），**不新增占号、不注册 handler、不提供实现** |
...
| F7 排期登记（Stage F 终审建议） | "commit() 失败路径泄漏 XID"（engine.rs:528-542：commit 失败时 handle 已消耗、XID 永留 active set 钉住 horizon）是 engine 侧真问题——本 stage 收口时显式排期处理（修复点：commit 失败时 best-effort abort 兜底，对齐 Stage O 的 commit 失败语义），不随 M3 关闭沉没。**回注（终审）**：实际已于 Stage F 收口期修复落地（commit 失败回退 best-effort abort + auto_commit 同路径 + `m2b_index_txn` 双路径红绿回归测试），本行仅作核销记录 |
...
| 全量回归 + release | debug + release 全量（含 m2b crash rounds、M1 crash_recovery；loom 命令列入但**预存红**（M2 出口提交二分同红，单开修复会话，见 stage_spec Stage D 条目），出口判定按其余项 + 诚实登记处理）；§12.5：100 并发 CRUD TPS 按 S2 协议对比 M2c 基线无统计显著回归（<5% 上限）。**回注（Stage G 收口）**：loom 预存红后经 slot-0 落位根治会话转绿（根因 = `pin_leaf_for_insert` 的 slot-0 判定在放闩窗口内陈旧），双模型 CI 口径全绿，出口判定不再依赖诚实登记——见 stage_spec Stage G 节 ✅ 根治条目 |
