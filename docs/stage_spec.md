...
### slice 4 交付内容（页驻查询 search,§10.2 任务 3 消费面 / §11.4"可查询"承载，2026-09-21)

1. **`HnswIndex::search`**(index.rs,pub):镜像 graph.rs Hnsw::search 的语义与**错误优先级**——dim 检查 → §5 入口校验（metric 自距离漏斗：有限性 + cosine 零向量）→ ef ≥ k → 空图早退 → k=0 早退 → PagedGraph 上跑泛型核（greedy 下降 ef=1 + level-0 beam)→ take(k) 输出 (NodeId, f64) 升序。ef=None 用 meta 钉死的 ef_search_default;rustdoc 立文 INITIALIZING 语义（§8.1③:state 位不进搜索路径）、单线程前提、目录损坏 fail-stop 前提（paged.rs 模块头口径）
2. **P3-B 闭合（零分配搜索热路径，slice 3 终审登记的评估点，结论 = 做）**:distance.rs 三度量迭代器变体（pub(crate);位级一致契约 = 同分量序 0..dim / 同 f32→f64 精确提升 / 同累加器序列，`to_bits()` 钉死；校验收窄为形状（长度相等 + dim≠0 + cosine 零向量）,**有限性不重复**——hot path 前提与 GraphAccess trait 契约呼应）;graph.rs `Metric::distance_iter` 单点分发（与 `Metric::distance` 同位，分发不双写）;PagedGraph::dist_to_query **零分配**(vector_iter 在 pin 守卫存活期内流式完成计算）,dist_between 2→**1** 分配（死锁纪律禁同时双 pin:a 物化丢 guard 后 b 流式）;**unsafe 切片重解释方案否决**（安全纪律 > 零拷贝，Stage E benchmark 若证分配为瓶颈再议）
3. **pg-engine `hnsw_search` 薄封装**(engine.rs：只读、无 WAL、无 flush 义务，rustdoc 对齐 hnsw_insert 形状）
4. **测试 +6**(pg-am-hnsw lib 142→148):`paged_search_matches_in_memory_bitwise`（承重钉——L2/Cosine/InnerProduct × {dim=4/N=200/m=4} + {dim=1/N=900/m=2 节点页目录页双溢出}，查询网格 4 查询 × k{1,3,10} × ef{None,k,k+1,500},**Outcome 对拍**：结果逐 (NodeId, to_bits) 全等**或**错误变体相同）+ `paged_search_error_parity`(dim 错/NaN/±inf/ef<k 同变体，cosine 零查询同 ZeroVector——错误优先级是契约的一部分）+ `search_recalls_initializing_nodes`(§8.1③ 残态钉：node0 LIVE 连边 → node1 INITIALIZING,FPI 残态 + mem::forget 崩溃 + 重开，search 召回 node1 为最邻近（L2 自距离 0)——state 位不进搜索路径是**有意语义**钉死，非遗漏）+ `search_early_exits`（空图任意 ef 答 []、k=0 答 [])+ distance.rs 迭代器位级钉 ×2（三度量 × 含 960-dim 五组向量 to_bits 全等；形状错配拒绝）;pg-engine m5_hnsw_create_open +1(`hnsw_search_is_bitwise_stable_across_a_crash`:12 向量各自自召回距离 0,mem::forget 崩溃重开后每查询结果逐位相同——slice 1 redo 重建的页与搜索路径正交）

**slice 4 验收**:agent-21 落码、主线 diff 逐行验收；`cargo test -p pg-am-hnsw` **202 绿**(148 lib + 3 对拍 + 5 属性 + 2+1+3+40 集成，基线 196 +6)、`cargo test -p pg-engine --test m5_hnsw_create_open` **5/5**(4→5)、clippy 双 crate -D warnings / fmt --check / doc 双 crate 零警告 / check --workspace 全绿（全部主线亲跑）。**Stage C 四个 slice 自此齐**(redo handler ×7 → 算法核泛型化 → insert → search);未 commit——等用户终审确认，message 前缀 `PHASE2-M5-StageC`

**slice 4 对抗审查回流（2026-09-21,agent-23 verdict **PASS**,1 P3 + 1 nano 当轮闭合）**:**P3-1（错误优先级序只被单故障弱钉）**——Outcome 对拍的 (Err,Err) 只比 discriminant，复合故障输入下两侧若校验顺序分叉会产出同变体不同消息，测试照绿；补三枚复合故障用例（dim+ef<k / NaN+ef<k / NaN+dim)，断言同变体 + 消息含首失败检查 fragment("components"/"non-finite"——"dim → §5 → ef≥k"序自此测试钉死，非仅直读承载）。**nano**：跨引擎句柄混用无防线（engine_B.hnsw_search(&index_from_A) 的页 id 在 B 的池上读错页，大概率响亮 Corrupted)——hnsw_insert/hnsw_search rustdoc 补"句柄不跨引擎"前提（BTreeIndex 同惯例）,M5 范围不做防线。**agent-23 自我更正登记**:slice 2 终审的"枚举序非契约"建议已被 v1.37/v1.39 终审正确推翻（admit-then-evict 使扩展集对枚举序敏感），本轮由 agent-23 自行登记，留存轮次事实。**探查为净（下轮勿重复）**：位级一致是结构性论证（同分量序/同 f64 提升/同累加器链，与 dim 无关无分叉面）非仅实证；镜像保真逐行一致（校验序、ef_search_default 取 meta 钉死值与 WARN 错配口径一致、search_layer(...)[0] 非空前提、take(k) 输出形态）;vector_iter 的 dim 守卫 + ExactSizeIterator 精确 size_hint;dist_between 双 pin 纪律守住（a 的 guard 即弃再 pin b);INITIALIZING 残态形状与 §8.2"5 中/5 后 6 前"窗口一致；自距离 ==0.0 断言稳定（同值 f32 自差恰为 0);stage_spec/coding-plan/tech-selection 数字与实测逐字吻合。验证：lib 148 绿、pg-engine 5/5、clippy/fmt/doc 绿（主线亲跑）

**slice 4 主线终审（2026-09-21，与 agent-23 攻击面不重复）**:**多 seed × 多度量 × 重开后对拍探针**（临时集成测试，公共 API only,16 配置 = 4 seed × {L2 dim=4、Cosine dim=6、InnerProduct dim=5} + 4 组极端 m=2,N=300–400；每配置 54 查询点 = 6 查询 × k{1,5,20} × ef{None,k,2k+3}，双断言 = 崩溃前页驻 vs 内存逐位 + mem::forget 崩溃重开后 vs 崩溃前逐位）:**864 对比较零分歧**，跑完即删——固定 seed 钉之外无侥幸，slice 1 redo 重建页 × slice 4 搜索的正交面经公共 API 闭合。**探查为净（下轮勿重复）**：腐坏邻居 id ≥ hwm 时 search_layer 的 visited 位图索引 panic = 与 P3-A 同类的 fail-stop（合法流不可达：写路径选值恒 < hwm、redo funnel 校验被引 id < HWM);6–7 窗口隐藏高层节点（max_level 滞后）下降区间以 meta 为准，与 §8.3① 弱化不变量一致；孤儿条目（3–4 窗）无映射不可达、幽灵映射（4–5 窗）无入边不可达，不污染答案（§8.3③);cosine 存储侧零向量合法流不可达（插入入口 + NodeInit redo funnel 双闸）,cosine_iter 的 ZeroVector 只可能在腐败页上触发（fail-stop 既定前提）;hwm as usize 的 64 位前提与内存形态同，非新面
...
**状态**:🚧 进行中——slice 1(audit.rs + redo txn_id 前置门)✅ 已 commit(0675328);slice 2(mem::forget 窗口矩阵 + flush_to 末边界 P1)✅ 已 commit(caf8f22);slice 3(SIGKILL 崩溃轮次 + 四轮终审:活引擎锁钉/v1.55 恢复/rounds≥1 响亮/warnings 钉)✅ 已 commit(12154e1);slice 4(§11.3 ② 恢复后召回门)✅ 落码 + 主线验收 + agent-23 对抗审查 PASS(1 P3 当轮修复)(2026-09-23):**post-recovery recall@10 = 0.9990,与 M4 内存门逐位同值**、release 两测 88.9s 全绿、ci.yml recall-gate job 注册落地;未 commit——等用户终审确认,message 前缀 `PHASE2-M5-StageD`
...
### slice 2 交付内容(mem::forget 窗口矩阵,2026-09-22)

1. **设计裁决**:§11.1/coding-plan 的 InsertState stepper 为条件句("多步协议若需暴露内部步骤")——HNSW insert 刻意单体(§8.4 不立 CLR/tracker 等价物),改采**故障注入栅栏**:窗口残态由真实生产 insert 产出,对手工构造零漂移
2. **index.rs 探针(test-only)**:ProbeMark 九变体(doc hidden;NodePageInit/DirPageInit/DirLink/NodeInit/DirAppend/NeighborEdge{level,shrank}/OwnList{level}/MetaUpdate/PublishLive)+ ProbeState(RefCell;marks/crash_after/logging)+ probe_marks/probe_clear_marks/probe_set_crash_after/probe_set_logging 访问器 + barrier——到界先 flush_to(lsn) 再 Err "crash probe"(崩溃可见性=持久前缀;LsnClock::current()=下一未分配 LSN=末记录末尾偏移,flush_to 取等合法);insert 的 9 类追加边界、10 个插桩点(NodePageInit 两个分支各一),记录序列与语义零改动(alloc_* 签名不动)
3. **矩阵抓到的真 bug(已修)**:open() 的 current_node_page 解析用"链尾页+(hwm-1)%813"——空链尾残态(§8.2"2 后 3 前"窗口,DirLink 后崩溃)下 dir_entry 越界、open 响亮失败;修为 `pages[(hwm-1)/813]` + `(hwm-1)%813`(与 PagedGraph::resolve 同公式,index.rs 注释立文)。`window_empty_linked_tail` 对该修复承重(旧代码下 recover 必红)
4. **tests/m5_insert_crash.rs(新建,876 行)**:13 枚 = §8.2 表 10 行(1–2 行拆节点页/目录页孤儿两枚、6–7 行拆隐藏高层/首节点修复两枚)+ probe_log_shape 确定性钉(seed 0xC0FFEE 前两抽 [0,0] 的精确 mark 序列)。模式:目录 A 探针发现(全程 marks 日志)→ 目录 B 同序重放(逐 insert marks 相等 sanity 钉决定论前提)→ crash_after 触发 → mem::forget(engine+index)模拟 kill -9 → open_with_redo_handlers + open + §11.3 audit → 行断言(孤儿/幽灵/hidden 计数、未发布 NodeId 复用、search 逐位稳定或合法性零容差距离重算)→ 二次 forget 重开证重放幂等(两次报告相等)

**slice 2 验收**:agent-21 落码(2h 超时一次,resume 续跑完成)、主线 diff 逐行验收 + 全量亲跑(225 绿 = 158 lib + 13 新 + 其余不变;clippy -D warnings / fmt / doc 绿)。813 填充两测(orphan_dir_page/empty_linked_tail)以 dim=4 + m=2/m_max0=4 压时,全矩阵 13 枚 ~22s

**slice 2 对抗审查回流(2026-09-22,agent-23 verdict **PASS**,1 P3 当轮修复 + 2 nano)**:**P3-1(生产路径 marks 无界增长)**——barrier 在 crash_after=None 时仍 borrow_mut+push,长寿命句柄 1M insert ≈ 16–24MB 常驻,与"生产零行为变化"声称不符;修复:ProbeState 加 logging 门(默认关),barrier 在"未布防 ∧ 未开日志"时纯 no-op 分支,声称由虚转真。**nano①**——RefCell 使 HnswIndex 变 !Sync:struct rustdoc 点名(Send 保持,与 §8.4 单线程前提一致);**nano②**——OpenOutcome 无显式修复信号(可见状态即信号)登记为设计选择。**探查为净(下轮勿重复)**:open() 修复公式在 check_dir_chain 不变量(ordinal 连续、count∈[0,813])下索引恒在界内(空尾/hwm=0/腐败链均有兜底);barrier borrow_mut 作用域先于 flush 释放,无 RefCell 重入;13 枚与 §8.2 表逐行对应、victim_pick 非平凡(幽灵行要求同 insert 有边、shrink 行要求被 shrink 边非末条);recover_twice 报告相等为确定性断言(append 同步 write,fsync 仅 OS 崩溃语义——同进程二次重放必见首轮修复记录;v1.53 终审修正);DataDirLock reclaim 分支放行同进程 stale lock,13 枚同进程互不干扰;probe_log_shape 的 seed 声称经探针实测属实。验证:lib 158 + m5_insert_crash 13 绿、clippy/fmt/doc 绿、全套件 225 复跑绿(主线亲跑)

**slice 2 主线终审(2026-09-22,攻击面与 agent-23 不重复:harness 级 WAL 并发面 + 窗口覆盖完备性)**:① **mem::forget 泄漏 engine 的 WAL worker 与二次重开的同文件并发面——探查为净**:`WalWriter::append` 在状态锁内同步 `write_record_to_segment`(wal/writer.rs:266,字节即时落文件),worker 的定时/批量路径只 **fsync**(writer.rs:564-600, dup 句柄、fsync 作用于 inode 不写字节);被 forget 的 engine 不再收到新 append,其泄漏 worker 至多对同 inode 多做幂等 fsync,与二次重开的 writer 无写字节交错——harness 无 WAL 竞争,同结论适用于 btree_split_crash 同型 harness。② **窗口覆盖完备性**:"8 后 flush 前"(PublishLive 已 append、成功边界 flush_to 未执行)非窗口表缺行——崩溃可见性 = 持久前缀(§8.1②),PublishLive 未持久 ⟺ 7–8 行残态,被 `window_meta_before_publish` 吸收;持久则等同成功(合法可见)。③ **措辞修正一处**:slice 2 对抗审查段的"recover_twice 修复记录已/未 flush 两路径"不精确(①的同步 write 事实下同进程重放必见修复记录,断言系确定性),已改并回改 coding-plan v1.52 同行。零代码变更。

**slice 2 主线二审 + agent-23 二轮回流(2026-09-22,均 verdict PASS,2 P3 + 2 nano 当轮闭合)**:**主线二审**——① P3:discover 的目录 A 泄漏(`let _ = crash(lab)` 丢弃路径,12 枚窗口测试各漏一个完整数据目录);修复:Discovery 携带 dir 字段、run_window 采收后即清。② nano:recover() 静默丢弃 open warnings,补断言。**探查为净**:崩溃受害者抽层不落盘、重开 skip-ahead 恰 hwm 次、续插重抽同层(与"未发布=未分配"一致);barrier 仅触发时 flush;m=2 无溢页面。**agent-23 二轮**——① **P3-1(续插后无二次审计)**:四枚 NodeId 复用行续插后图状态未钉;补续插后 audit(node_count/live==victim+1、initializing==0、hidden==0;orphan_entry 行 orphan==**1**——孤儿条目续插后仍存留被计,§8.3⑤ 记账钉死)。② **P3-2(edge_count 零断言)**:run_window 采受害者前 pre_report 基线 + edge_delta 辅助(反向边 +1、shrank 净 0),ghost_mapping(Δ=0)/partial_backward_edges(前缀恰 1 边)/shrink_interleave(前缀 #边−#shrink)/own_lists_empty(全部反向边)四行钉精确 edge_count——四行实测全绿,反向验证了 audit 边遍历通路与 insert 记录语义的边际计数一致;含自身层写入的两行(mark 不带列表长度)登记不钉精确值。**nano×2**:fresh_dir 同名残目录理论面(登记);probe_log_shape 反向判别力 = 协议形状钉(既定口径)。**探查为净(下轮勿重复)**:barrier Err 传播的资源态(9 点均在 apply+stamp 后,guard 正常 Drop,句柄必被遗忘);"crash probe" fragment 在 pg-am-hnsw 之外零命中(pg-engine 消费点 engine.rs:1449-1458 为透传);DirLink barrier 触发后的句柄残态与真实崩溃一致;fresh_dir 并发唯一性成立。验证:m5_insert_crash 13 绿、clippy/fmt/doc 绿(主线亲跑)

**slice 2 终审三轮(2026-09-22,用户审查回流:1 P1 系统性预存 + 2 nano,当轮闭合)**:**P1(flush_to 边界语义 doc/impl 偏一,预存系统性)**——flush_to 文档承诺"LSN ≤ lsn 的记录已 fsync",但 synced_lsn 只落记录末边界、append 返回起始 LSN,波次/重开后第一条记录的 flush_to(起始) 恰好早退,记录自身未 fsync;真 OS 崩溃才可观测(同进程/子进程 harness 页缓存恒可见),commit 路径后果 = 崩溃窗内恢复判 aborted 而客户端已见 committed。修复 = 全库统一末边界:flush_to rustdoc 前缀语义立文 + 钉测试(pg-storage)、CheckpointEnd flush 改末边界(checkpoint.rs)、逐出守卫条件 < 改 <= 且目标改 current_lsn()(buffer_pool.rs)、commit/abort 改 flush_to(start+record_size)(manager.rs,CommitWal trait 不动,commit_hard_order 断言改钉末边界)、insert 成功边界与 probe barrier 统一 current_lsn()(index.rs,barrier 去 lsn 形参——"同函数两种形态"线索消除)。**nano×2**:测试行数 756→876;"9 个 WAL 追加点"修正为 9 类边界/10 插桩点(NodePageInit 两分支)。验证:wal::writer 17 绿(含新钉)、commit_hard_order 4 绿、pg-am-hnsw lib 158 + 窗口矩阵 13 绿、全 workspace 回归、clippy/fmt/doc(主线亲跑)

### slice 3 交付内容（SIGKILL 崩溃轮次，2026-09-23)

1. **crates/pg-engine/tests/m5_hnsw_crash_rounds.rs（新建，四轮终审后 520 行）**:m2b_crash_rounds 模式的引擎级真 SIGKILL 轮次——父进程 spawn 测试二进制自身为子（`M5_CRASH_CHILD=1` 门控），子进程经引擎公共 API(create_hnsw_index → open_hnsw_index → hnsw_insert ×120+below(60)，每 47 次一 checkpoint,NodeId 稠密钉）跑确定性 insert 流，每次 insert Ok 后原子重写 expectation.txt(MODE/OPS/OID,tmp+sync_all+rename)；偶数轮 kill 于 ready-to-die 后（full)，奇数轮杀在 OPS ≥ 30+(round%60) 进度点（mid)。`M5_CRASH_ROUNDS` 默认 25(CI，随 pg-engine 既有 test matrix 跑，无专用 job——v1.1 事实口径）、1000 验收手动。
2. **父进程验证集**:stale lock 删除（m2b 先例）→ Engine::open(redo 重放）→ OID 目录解析 → open(warnings 空钉）→ **§11.3 audit 每轮必过**;full 模式 = 计数全等（node_count/live==n、三类残态与 tombstone 全 0)+ **内存 Hnsw 孪生位级比对**(vector_at 序无关纯函数父子逐字节一致，4 查询 × k{1,5,15} 的 (NodeId,dist.to_bits()) 序列逐位等，§11.1 同平台口径）;mid 模式 = 前缀耐久（live/hwm ∈ {n,n+1}、hwm−live ≤ 1)+ 残态上界（initializing/orphan/hidden 各 ≤1、tombstoned==0)+ 可达残态一致性钉（ghost⟹hwm=n+1、orphan⟹hwm=n、hidden⟹ghost——单在飞 insert 的全部可达残态形）；零残态仍做孪生比对（m=live，含"insert 已耐久、expectation 未写"的 n+1 形），有残态只做功能 sanity（幽灵半边连边合法偏离任何干净重放，模块头立文）。selection 按种子奇偶交替覆盖 Heuristic/Simple 双路径。
3. **主线实测**:25 轮 74.7s 全绿（~3s/轮）;13 奇数轮残态分布 = **10 幽灵**（杀在 DirAppend 后 PublishLive 前的在飞窗）+ 1 零残态 live=n+1(expectation 竞态形）+ 2 干净；**orphan/hidden 未命中**（窗口窄：孤儿 = NodeInit→DirAppend 微窗，hidden 需 level>max 的在飞 insert——slice 2 窗口矩阵已确定性覆盖这两类，如实登记非缺口）。

**slice 3 验收**:agent-21 落码（两次 2h 超时——原因为 25 轮全量跑的等待，非卡死；主线接管计时实测后排除了性能疑点）、主线 diff 逐行验收 + 全量亲跑（25 轮 74.7s 绿、fmt 后 4 轮复跑绿、**release 档 4 轮 8.3s 冒烟绿**（验收命令的真实形态，~2s/轮，1000 轮 ≈ 33min 与 m2b 估算同量级）、clippy -D warnings / fmt 绿）。**主线独立二轮（用户请求，攻击面与前两轮不重复）**:checkpoint 邻域命中观察（round 17 target=47 恰等于 checkpoint 点 i=47,kill 落在 checkpoint ±1 操作邻域；**无 checkpoint 内 marker，前/中/后不可判定——如实登记为"邻域命中"而非 Begin/End 之间的实证**；奇数轮 target ∈ [30,89] 跨 i=47,checkpoint 前/后两 kill 区域均有轮次覆盖）、轮次间状态隔离（engine 先于 TempDir drop，无 slice 2 同型泄漏）、catalog 行耐久先于 kill 点、kill 不可能落在 create/open 期间、mid sanity 下界论证（entry point 恒为旧入口、已完成前缀图连通）、full 模式 kill 点为静止态——其余全净。

**slice 3 对抗审查回流（2026-09-23,agent-23 verdict **PASS**,1 P3 当轮闭合 + 2 nano)**:**P3-1({n,n+1} 上界的时序论证环境相关）**——模块头原称"every child op costs several fsyncs"，实为恰 2 次（§8.1① WAL flush + expectation sync_all)；上界成立的真正前提 = 单次 insert+expectation 周期 > 2ms 轮询窗，真实磁盘恒成立（实测 ~ms 级），亚毫秒 fsync 环境（tmpfs/极快 NVMe）单窗可容多次 insert → 界可破。处置：模块头改精确口径立文（含放宽界/慢轮询两个旋钮；m2b 同前提，接受不修码）。**nano①**:sync_all→rename 之间被杀遗留 expectation.tmp，父只读 .txt，无害（注释立文）;**nano②**：子进程 stdout/stderr 置 null,panic 只能靠父 120s 超时定位（m2b 同形，登记不改）。**探查为净（下轮勿重复）**：零残态⟹完整前缀（八步序任何中点必留三类残态之一，twin(live) 比对对象正确）;full 模式 hidden==0 恒成立（was_empty 首节点无条件 MetaUpdate + checkpoint 截断两形态）；期望文件读竞态干净（rename 原子、缺目录 fsync 仅 OS 崩溃语义相关）;checkpoint 不动 rng 流（open 按链导出 hwm skip-ahead 重推）;NodeId 稠密钉无误杀面；target ≤ 89 < 最小 op_count 120;mid sanity hits.len()==5 由 live ≥ 30 保证。

**slice 3 用户终审三轮（2026-09-23，用户请求复核，2 nano 当轮闭合 + 1 nano 登记）**:**nano①(hwm 双源未交叉钉）**——mid 模式的一致性钉用句柄 `index.hwm()`(open 时 DirChainInfo 导出），而 audit 内部不变量用 audit 自己重走链导出的 node_count，两个独立链遍历各信各的、漂移不红；verify 在模式分叉前补一行交叉钉（`report.node_count == index.hwm()`,open 与 audit 之间零变更，任何分歧即 bug)。**nano②**:`spawn_child` 的 `--nocapture` 是死参数（子进程 stdout/stderr 已 Stdio::null)，摘除。**登记不改（m2b 先例）**:`M5_CRASH_ROUNDS` 非法值静默回退 25（测试 env var,m2b 同型；M4"响亮失败"纪律适用于产品面）。**探查为净（下轮勿重复）**:open-repair 首节点修复窗在本 harness 不可达（target ≥ 30 ⟹ 首 insert 早已完成，slice 2 已确定性覆盖该窗）;entry_point/max_level 由 audit c 组 + 孪生位级比对行为性覆盖；edge_count 不另钉（位级比对是更强断言）;checkpoint i=94 仅 even 轮经过（target ≤ 89)，属覆盖性质观察非缺口；`M5_CRASH_CHILD=1` 误设时 child entry 缺 DIR 响亮 panic。验证：fmt 绿、6 轮 17s 复跑绿（主线亲跑）;stage_spec 本段同步。

**slice 3 用户终审四轮（2026-09-23，外部 review verdict FAIL,**1 P1 + 2 P2 + 2 P3 全部属实**，当轮闭合）**:**P1（偶数 full 轮并未 SIGKILL 活引擎）**——`run_child` 返回后才写 ready-to-die,Engine/HnswIndex 随作用域结束析构，`DataDirLock::drop`(data_dir_lock.rs:99-105）删锁；外部探针实证"marker 在、子进程活、lock 已无"——12 个偶数轮杀的是引擎已拆除的进程，文档声称的活引擎 SIGKILL 为虚。修复：marker + 无限等待移入 `run_child`（改 `-> !` 契约，注释立文"引擎必须在作用域内直到 kill")，并补**锁存在性钉**(kill 后 lock 必须在——SIGKILL 无 Drop，锁在 = 活引擎被杀；该断言把 P1 形态焊死不可回归）。**P2-a(coding-plan v1.55 被覆盖）**:v1.56 落码时 Edit 误替换而非追加，v1.55(flush_to P1 修复史）丢失；从 git 恢复 v1.55 原行并重新追加 v1.56，零信息损失。**P2-b(checkpoint 实证声称不成立）**:round 17 target=47 恰等于 checkpoint 点，但无 checkpoint 内 marker,kill 在前/中/后不可判定——二轮回流段的"实证覆盖 Begin/End 之间"改为"邻域命中"如实口径；不加 checkpoint 内故障注入（窗口矩阵归 slice 2 职责）。**P2-c(M5_CRASH_ROUNDS=0 静默绿）**：实测 0 轮 0.00s 全绿 vacuous pass；改响亮失败（非法值 panic、rounds ≥ 1 断言）——与 m2b 静默回退先例分岔，登记理由（vacuous green 比响亮失败更糟）。**P3-1**:verify_round 的 warnings 空钉此前是虚声称（`.index` 丢弃 warnings)——补显式断言 + 漂移理由注释。**P3-2**：行数引用 488→520（四轮修复后实测）。验证：全量 25 轮复跑绿（含锁钉生效）、fmt 绿（主线亲跑）;coding-plan v1.57 同步。

### slice 4 交付内容（§11.3 ② 恢复后召回率门，2026-09-23)

1. **crates/pg-am-hnsw/tests/m5_recall_after_recovery.rs（新建）**：三阶段崩溃形态——建半段（5000)→ mem::forget 崩溃（无 checkpoint)→ redo 重开 → 续插至 10000(NodeId 稠密钉，skip-ahead 重同步在位）→ audit 精确钉 → 采崩溃前查询答案 → 再崩溃 → 重开 → audit 精确钉（node_count/live=10000、三类残态与 tombstone 全 0)→ 100 查询逐查询 `(NodeId, dist.to_bits())` **位级相等**（恢复对检索行为零改变）+ recall@10 ≥ 0.98(§11.3 ② 硬门，冻结 M4 参数 M=16/m_max0=32/efC=200/ef=64/seed=42/L2，与内存门同参可比）。
2. **常驻伴随 synthetic**(800×8d MixtureGen，本地 brute-force 真值）：同三阶段形态，松阈值 0.90——无数据集矩阵 job 不做纯 skip 占位（recall_siftsmall 同纪律）;**强钉是位级前后相等，非 recall 数**。
3. **CI 注册落地（coding-plan 清单 #11)**:ci.yml recall-gate job 新增 `Post-recovery recall hard gate (M5)` 步（`M4_REQUIRE_DATASET=1` 步骤级 env，与 recall_siftsmall 同 job 同触发，YAML 校验通过）——§11.3 ② 自此在 CI 有覆盖，不重演 M4"绿但没跑"。

**slice 4 验收**:release 两测全绿 88.9s（主线亲跑 ×2);**post-recovery siftsmall recall@10 = 0.9990，与 M4 内存门（0.9990）逐位同值**——恢复对检索质量零损耗的实测证据；synthetic 0.9700 ≥ 0.90。fmt/clippy 绿。

**slice 4 对抗审查回流（2026-09-23,agent-23 verdict **PASS**,1 P3 当轮修复 + 1 P3 登记 + 2 nano)**:**P3-1（临时目录从不清理）**——每次运行在 /tmp 累计 ~40MB（页 + WAL)，注释自称与 m5_insert_crash 同模式却未兑现清理半边；修复：phase 3 末 shutdown + remove_dir_all（实测零残留）。**P3-2（证明结构非对称，登记不改码）**:pre_crash 答案采于第一次恢复后的索引，位级相等严格证"第二次恢复幂等"，第一次恢复正确性由 audit（结构）+ recall 阈值背书——≤1.9pp 松弛内的稳定劣化理论在覆盖外；实测 0.9990 逐位锚定实际风险近零，收紧阈值会引入 flaky，否决。**nano①**:M4_REQUIRE_DATASET 非 "1" 值静默 skip——与 recall_siftsmall 既有口径逐字一致（NotUnicode 已比旧文件更严，响亮 panic)，登记不改。**nano②（盲区登记，不加）**:"恢复过程自身被杀"(kill-during-redo、修复幂等性的再崩溃形态）本文件未覆盖——后续候选。**探查为净（下轮勿重复）**：本地 l2_squared 与 Metric::L2 同累加序同 f64 提升、(dist,id) 排序与 ivecs 原序口径自洽；三文件同时存在性门、长度钉、recall_at_k 自带完整性校验；recover() 两调用点均为已完成建图、精确钉无合法误杀形态；fresh_dir pid+counter+tag 并发唯一；mem::forget 同进程 stale lock 由 DataDirLock reclaim 分支刻意放行；ci.yml 步骤级 env 不外泄、位于 fetch/cache 之后、--release 口径与测试头注一致。

**slice 4 主线终审（2026-09-23，用户请求 review，攻击面与 agent-23 不重复）**:**P3-2 实质闭环**（对抗审查登记项）——原证明结构里 pre_crash 答案采于第一次恢复后的索引，位级钉严格只证"第二次恢复幂等"；补**未崩溃孪生位级钉**(phase 3 末：内存 Hnsw 同参数（16/32/200/64/seed 42）同向量流重放，全部 100 查询 + synthetic 20 查询与恢复后答案逐位等），两次恢复自此都锚在从未崩溃的参照上——并在 **10k 真实数据规模**首次实测跨形态（页驻 ↔ 内存）位级 parity（此前最大 950/150 节点），零分歧，recall 仍 0.9990。**debug 档计时实测**（新数据点）：两测 349.9s(twin 钉约再 +1 分钟）——本地带数据集跑 debug 全套件的成本；CI 矩阵无数据集必 skip、recall-gate 走 release，不受影响；登记不改码（m2b crash rounds 25 轮 debug 同量级先例）。**nano**:phase 2 续插后的 audit 只钉 node_count 未钉全套字段（残态由 phase 3 recover 的全钉兜底，登记）。验证：release 两测 114.4s 全绿（含孪生钉）、debug 两测 349.9s 全绿、clippy -D warnings / fmt 绿（主线亲跑）;coding-plan v1.59 同步。

**slice 4 终审四轮（2026-09-23，用户请求再 review，主线独立审 + agent-23 第四轮对抗审查，双 verdict **PASS**，零 P1/P2/P3,3 nano 登记不改码）**:**nano①（口径登记）**——位级钉比较查询输出 `(NodeId, dist.to_bits())` 而非全图拓扑；理论可存活的边级劣化（过 audit、不改查询答案、recall 达标）由审计（结构）+ recall（质量）+ Stage C slice 3/4 拓扑对拍（N=200/900 位级）组合兜底；本文件定位 = §11.3 ② 召回门，接受；10k 全拓扑对拍列为可选增强。**nano②（有意行为）**——phase 3 清理（shutdown + remove_dir_all）仅成功路径执行，断言失败时 panic 残留 ~40MB 现场——失败调试所需，登记不改。**nano③（CI 登记）**——recall-gate 无 timeout-minutes（默认 360min ≫ 需要，沿用既有形状）;M4 门第一步失败时 M5 第二步被 `if: success()` 默认跳过（job 已红无假绿，仅失诊断信号）——可选 `if: always()`，非必须。**第四轮亲验为净（下轮勿重复）**：假绿面无退化相等路径/阈值方向正确/无早退，"四次恰同值"是确定性预期；孪生钉同参同种子由共享 Cfg 值对象代码级保证、ef 两侧显式 Some(64);mem::forget 可见性模型与进程崩溃一致；两测试 tag/COUNTER/pid 隔离；CI 步骤级 env 合并不遮蔽；文档与 371 行实现、ci.yml、实测数字（0.9990/0.9700、release 88.9~114.4s、debug 349.9s）逐字一致零漂移。零代码变更，无需复跑；coding-plan v1.60 同步。

**slice 4 终审五轮（2026-09-23，用户终审 1 P3，核实属实当轮闭合）**:**P3-1(synthetic 真值重复实现）**——本地 `l2_squared` 的"crate-private 故重写"理由只对 enum 方法成立，`distance::l2_squared` 是 pub 自由函数（`Metric::L2` 即委派它）；两份实现今日位级同，但 crate 累加纪律变动时真值与索引度量静默分叉、仅靠 0.90 松阈值兜底。修复：删本地 fn，导入 `pg_am_hnsw::distance::l2_squared` + `.unwrap()`（白得 validate_pair 形状/有限性校验），注释立文"真值永不与索引度量分叉"。验证：fmt / clippy -D warnings 绿、release 两测复跑（主线亲跑）;coding-plan v1.61 同步。
